...
EmberEventControl outputBottonEventControl;
...
static bool LigarLedBotao = false;
/*** LED ***/
...
void inputActionEventFunction(void)
{
	...
	if(buttonPressed){
		...
		if(resetCounter != true) {
			...
			resetCounter = true;
			emberEventControlSetActive(outputBottonEventControl);
			emberEventControlSetDelayMS(inputActionEventControl, 5*1000);
		} else {
			//Reset Device
...
void outputBottonEventFunction(void)
{
	if(LigarLedBotao == false) {
		halToggleLed(LED_OUTPUT);
        LigarLedBotao = true;

	} else if (resetCounter == true){
		halToggleLed(LED_OUTPUT);
	      //LigarLedBotao = false;
	}
	else {}
